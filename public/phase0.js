...
const teams = [
    ...
    { name: "Equipe 6", score: 0 },
    { name: "Equipe 7", score: 0 },
    { name: "Equipe 8", score: 0 },
  ];
  ...
  async function loadQuestions() {
    try {
      const response = await fetch('epreuve0.txt');                       // get the file
  ...
  /* it render the teams ins resultas, each time its called it sort and render them,
   ...
   function renderTeams() {
    ...
    teams.forEach((team, index) => {  //  create list item for each team
      ...
      if(team.score > maxpoints) team.score = maxpoints;
      ...
      } else if(team.score>0 && (team.score=== topScore || team.score>=maxpoints)&& winners.length <=numberOfWinners) {  // If there is no tie, and this is the first team, mark it as the winner
  ...
  document.getElementById("validateNextBtn").addEventListener("click", () => {
    ...
    if (team) {
      ...
      if (winners.length === numberOfWinners) {
        ...
        localStorage.setItem("winners12", JSON.stringify(winners)); // you can delete this, it just pass the winners to another page
        localStorage.setItem("losers12",JSON.stringify(loosers))
  ...
  function startTimer() {
    ...
    timerInterval = setInterval(() => {                                   // start the timer
      ...
      if (timeLeft <= 7) {                                               // if the time left is less than 20 seconds, put it in a warning state                   
        ...
        playSoundClock();                         // add the warning state
