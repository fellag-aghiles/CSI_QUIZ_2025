// Data & Initialization
const winnerssssssss1 = JSON.parse(localStorage.getItem("winner3")); // show the winner in the console
...
*/
let teams = []
winnerssssssss1.forEach((team) => {
  teams.push({ name: team, score: 0 }); // add the winners to the teams array with score 0
});// get the teams from the local storage
// remove the winners from the local storage, to avoid showing them in the next page
// remove the winners from the local storage, to avoid showing them in the next page

let selectedTeamName = null;              // selected team name
let currentQuestionIndex = 0;             // show the question number for the team
const maxTime = 15;                      // max time for the timer
...
const maxpoints = 2;  
const numberOfWinners = 1; // Number of winners to end the round
let clockAudio;
...
async function loadQuestions() {
  try {
    const response = await fetch('speciale.txt');                       // get the file
...
document.getElementById("validateNextBtn").addEventListener("click", () => {
  ...
  if (team) {
    ...
    if (winners.length === numberOfWinners) {
      ...
      localStorage.setItem("winner3", JSON.stringify(winners)); // you can delete this, it just pass the winners to another page
...
startStopBtn.addEventListener("click", () => {
  if (!timerRunning) { 
    showQuestoin();                                                   //  if the timer is not running, start it
    startTimer();
  } else {  
    maskQuestoin()                                                                //  if the timer is running, stop it
    stopTimer();
  }
});
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {                                   // start the timer
    ...
    if (timeLeft <= 7) {                                               // if the time left is less than 20 seconds, put it in a warning state                   
    ...
    if (timeLeft <= 0) {                                             // if the time left is 0                     
      currentQuestionIndex = -1;                                     // reset the question index (-1 cause it will be incremented )
      ...
      stopSoundClock();                                             // stop the sound                                                      // if we are not in the end, then show that the time is up, normally
