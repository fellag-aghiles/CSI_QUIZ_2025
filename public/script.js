...
document.getElementById("validateNextBtn").addEventListener("click", () => {
  ...
  if (team) {
    ...
    if (winners.length === numberOfWinners) {
      ...
      localStorage.setItem("winners11", JSON.stringify(winners)); // you can delete this, it just pass the winners to another page
      localStorage.setItem("losers11",JSON.stringify(loosers))
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {                                   // start the timer
    ...
    if (timeLeft <= 0) {                                             // if the time left is 0                     
      ...
      stopSoundClock();                                             // stop the sound if its playing
